...
public class Main {
    ...
    public static double getTimeSearch(List<Data> randomList, Queue<Data> collectionQueue){
        ...
        for (Data model  : randomList){
            ...
            for (int i = 0; i < collectionQueueCopy.size(); i ++){
                if (i == index){
                    collectionQueueCopy.element();
    ...
    public static double getTimeRemove(List<Data> randomList, Queue<Data> collectionQueue){
        Queue<Data> collectionQueueCopy = collectionQueue;
        List<Data> sortedRandomList = new ArrayList<Data>(randomList);
        Collections.sort(sortedRandomList, (d1, d2) -> {
            return (int) (d1.getId() - d2.getId());
    ...
    public static void main(String[] args) {
        ...
        double[][] tiemposSearch = new double [2][3];
        ...
        for (int i = 0; i<2; i++){
            System.out.printf( "%-15s", listOfTypeQueue.get(z).getClass().getSimpleName());
            for (int j = 0; j<3; j++){
                tiemposSearch[i][j] = getTimeSearch( listOfElementsToFind.get(j), listOfTypeQueue.get(z));
        ...
        double[][] tiemposRemove = new double [2][3];
        ...
        for (int i = 0; i<2; i++){
            System.out.printf( "%-15s", listOfTypeQueue.get(z).getClass().getSimpleName());
            for (int j = 0; j<3; j++){
                tiemposRemove[i][j] = getTimeRemove( listOfElementsToFind.get(j), listOfTypeQueue.get(z));
